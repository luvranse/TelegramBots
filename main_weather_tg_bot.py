...
async def get_weather(message: types.Message):
    ...
    try:
        ...
        if weather_description in code_to_smile:
            ...
            wd = "Посмотри в окно, там что-то непонятное!"
        ...
        await message.reply(
            f"\U0001F557{datetime.datetime.now().strftime('%H:%M %d-%m-%Y')}\U0001F557\n"
            f"{wd}\n"
            f"Погода в городе: {message.text}\nТемпература: {cur_weather}C°\n"
            f"Влажность: {humidity}%\nДавление: {int(pressure / 1.3568)} мм.рт.ст\nВетер: {wind} м/с\n"
            f"Восход солнца: {sunrise_timestamp.strftime('%H:%M:%S')}\nЗакат солнца: {sunset_timestamp.strftime('%H:%M:%S')}\nПродолжительность дня: {lenght_of_the_day}\n"
            )
